...
function Dashboard() {
    ...
    const handleDeploy = async (id) => {
        ...
        prev.map((w) =>
          w._id === id
            ...
            setError(error.response?.data?.message || "Unable to deploy the website")
        }
    }
    ...
    const handleDelete = async (site) => {
        const confirmed = globalThis.confirm(`Delete "${site.title}"? This cannot be undone.`)
        if (!confirmed) return
        ...
        try {
            await axios.delete(`${serverUrl}/api/website/delete/${site._id}`, { withCredentials: true })
            setWebsites((currentWebsites) =>
                currentWebsites.filter((website) => website._id !== site._id)
            )
        } catch (error) {
            setError(error.response?.data?.message || "Unable to delete the website")
        } finally {
            setDeletingId(null)
        }
    }
                            ...
                            >
                                <div className='relative h-40 bg-black cursor-pointer'  onClick={()=>navigate(`/editor/${w._id}`)}>
                                    ...
                                    <button
                                        ...
                                        className='absolute top-3 right-3 z-10 p-2 rounded-lg bg-black/70 border border-white/15 text-zinc-300 hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/30 transition disabled:opacity-50 disabled:cursor-not-allowed'
                                    >
                                        <Trash2 size={16} />
                                    </button>
                                </div>
